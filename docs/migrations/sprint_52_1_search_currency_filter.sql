-- Sprint 52.1: add original-currency filter to production search groups RPC.
...
DROP FUNCTION IF EXISTS search_active_offer_groups(jsonb, integer, text, integer, integer, boolean, boolean, text);
...
BEGIN
    WITH filtered AS (
        SELECT
            ...
            o.dealer_id,
            o.watch_id,
            offer_market_usd_price(o.usd_price, o.price_review_status, o.corrected_usd_price) AS usd_price,
            o.condition,
        ...
        WHERE o.status = 'active'
          AND offer_includes_market_price(o.price_review_status)
          AND (
          ...
          AND (
              ...
              OR (
                  offer_market_usd_price(o.usd_price, o.price_review_status, o.corrected_usd_price) IS NOT NULL
                  AND offer_market_usd_price(o.usd_price, o.price_review_status, o.corrected_usd_price) <= max_usd_price
              )
          )
          AND (
...
COMMENT ON FUNCTION search_active_offer_groups(jsonb, integer, text, integer, integer, boolean, boolean, text) IS
    'Return one page of brand+reference search groups with aggregate offer stats.';

REVOKE ALL ON FUNCTION search_active_offer_groups(jsonb, integer, text, integer, integer, boolean, boolean, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION search_active_offer_groups(jsonb, integer, text, integer, integer, boolean, boolean, text) TO service_role;
GRANT EXECUTE ON FUNCTION search_active_offer_groups(jsonb, integer, text, integer, integer, boolean, boolean, text) TO authenticated;
GRANT EXECUTE ON FUNCTION search_active_offer_groups(jsonb, integer, text, integer, integer, boolean, boolean, text) TO anon;
